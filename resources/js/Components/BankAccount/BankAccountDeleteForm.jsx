...
const BankAccountDeleteForm = ({ bankAccount }) => {
    const {closeModal} = useModal();
    const { delete:destroy} = useForm()
    const {displayNotification} = useNotification();

    const handleDelete = (e) => {
        destroy("/bank-accounts/" + bankAccount.id, {
            onSuccess: () => {
                closeModal()
                displayNotification("Votre compte bancaire a bien été supprimé.", "success", "bankAccount");
            },
            onError: () => {
...
export default BankAccountDeleteForm;
